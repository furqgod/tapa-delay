...
class StreamDelayServer extends EventEmitter {
    constructor() {
        ...
        // Platform configs
        this.platforms = {
            twitch: { enabled: true, key: '', server: TWITCH_SERVERS.sa_east, stableMode: true, manuallyStopped: false },
            youtube: { enabled: false, key: '', server: YOUTUBE_SERVER, stableMode: true, manuallyStopped: false },
            kick: { enabled: false, key: '', server: KICK_SERVER, stableMode: true, manuallyStopped: false },
        };

        this._spawnWriter = null; // referência ao spawnWriter da pipeline ativa
    ...
    startPlatform(platform) {
        if (!this.platforms[platform]) return { success: false };
        if (!this.connected) return { success: false, error: 'Stream não ativa' };
        const p = this.platforms[platform];
        if (!p.key) return { success: false, error: 'Stream key não configurada' };
        p.manuallyStopped = false;
        if (this._spawnWriter && !this.writerProcesses[platform]) {
            this._spawnWriter({ name: platform, url: `${p.server}${p.key}`, stableMode: p.stableMode });
        }
    ...
    _startPipeline(streamPath) {
        ...
        const spawnWriter = (output) => {
            ...
            // Vídeo: passthrough (zero re-encode). Áudio: re-encoda por writer para corrigir timestamps
            writerArgs.push(
                '-map', '0:v:0',
                '-map', '0:a:0',
                '-c:v', 'copy',
            ...
            writer.on('close', async (code) => {
                ...
                const unexpected = code !== 0 && this.connected && !this.platforms[output.name]?.manuallyStopped;
                ...
                if (this.connected && !this.platforms[output.name]?.manuallyStopped) {
    ...
    _stopPipeline() {
        ...
        // Reseta flags manuais ao encerrar a stream

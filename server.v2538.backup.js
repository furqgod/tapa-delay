...
class StreamDelayServer extends EventEmitter {
    constructor() {
        ...
        this._reconnectTimers = {}; // timeouts de auto-reconexão por plataforma
    ...
    startPlatform(platform) {
        ...
        // Cancela qualquer timer de reconexão pendente antes de spawnar
        if (this._reconnectTimers[platform]) {
            clearTimeout(this._reconnectTimers[platform]);
            delete this._reconnectTimers[platform];
        }
        if (this._spawnWriter) {
            // Mata writer existente (se houver) para garantir spawn limpo
            if (this.writerProcesses[platform]) {
                this.writerProcesses[platform].kill('SIGKILL');
    ...
    _stopPipeline() {
        ...
        // Cancela todos os timers de reconexão pendentes
